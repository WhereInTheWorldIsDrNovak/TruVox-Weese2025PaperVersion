...
const Heteronym: React.FC<HeteronymProps> = ({
  ...
  const updatePitchArrayCus = () => {
    ...
    for (let i = 0.5 * partLength; i < 1.5 * partLength; i++) {
      updatedArray[i] =
        inputValue[1] +
        (midValue - inputValue[1]) *
          ((i - (1.5 * partLength + offset)) / partLength) -
        50;
    }

    for (
      let i = 1.5 * partLength + offset;
      i < 2.5 * partLength + offset;
      i++
    ) {
      updatedArray[i] =
        midValue +
        (inputValue[1] - midValue) * ((i - 0.5 * partLength) / partLength) -
        50;
    }
